...
class UndirectedWeightedGraph:
	...
	def _get_minimum_temporary(self):
		...
		for i in range(self.nvertices):
			if self.vertex_list[i].status==TEMPORARY and self.vertex_list[i].length<min:
				min = self.vertex_list[i].length
				k = i
	...
	def _prims_algorithm(self, r):
		...
		for i in range(self.nvertices):
			...
			self.vertex_list[i].length = INFINITY
		
		# Make length of source vertex equal to 0
		self.vertex_list[r].length = 0
		...
		while True:
			...
			# Insert the edge (vertex_list[current]->predecessor,current) into the tree,
			# except when the current vertex is root
			if current != r:
				count += 1
				self.tree_edges.append(Edge(self.vertex_list[current].predecessor,current))
				
			for v in range(self.nvertices):
				# Checks for adjacent temporary vertices
				if self._is_adjacent(current,v) and self.vertex_list[v].status==TEMPORARY:
					if self.adj[current][v] < self.vertex_list[v].length:
						self.vertex_list[v].predecessor = current	# Relabel
						self.vertex_list[v].length = self.adj[current][v]
	...
	def minimum_spanning_tree(self, root):
		...
		print("Root vertex : " + self.vertex_list[r].name)
