...
class DirectedGraph:
	...
	def _bfs(self, vertex):
		...
		self.vertex_list[vertex].predecessor = NIL
		self.vertex_list[vertex].distance = 0
		...
		while bfs_queue.qsize() != 0:
			...
			for i in range(self.nvertices):
				...
				if self._is_adjacent(vertex,i) and self.vertex_list[i].state==INITIAL:
					...
					self.vertex_list[i].distance = self.vertex_list[vertex].distance + 1
	...
	def bfs_traversal(self, vertex_name):
		...
		for i in range(self.nvertices):
			...
			self.vertex_list[i].distance = INFINITY			
	...
	def bfs_shortest_path(self, source, destination):
		...
		if self.vertex_list[self._get_index(destination)].distance == INFINITY:
			print("There is no path from " + source + " to " + destination)
		else:
			print("Shortest distance is :", self.vertex_list[self._get_index(destination)].distance)
			...
			while y != NIL:
				path.append(y)
				x = self.vertex_list[y].predecessor
				y = x
