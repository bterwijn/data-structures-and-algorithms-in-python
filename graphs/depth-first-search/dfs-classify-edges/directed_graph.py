...
class DirectedGraph:
	...
	def _dfs(self, vertex):
		...
		self.vertex_list[vertex].discovery_time = DirectedGraph.time	# discovery time
		...
		for i in range(self.nvertices):
			if self._is_adjacent(vertex,i):
				...
				elif self.vertex_list[vertex].discovery_time < self.vertex_list[i].discovery_time:
					print("Forward Edge - (" + self.vertex_list[vertex].name + "-" + self.vertex_list[i].name + ")")
				else:
					print("Cross Edge - (" + self.vertex_list[vertex].name + "-" + self.vertex_list[i].name + ")")
		
		self.vertex_list[vertex].state = FINISHED
		DirectedGraph.time += 1
		self.vertex_list[vertex].finishing_time = DirectedGraph.time	# finishing time
