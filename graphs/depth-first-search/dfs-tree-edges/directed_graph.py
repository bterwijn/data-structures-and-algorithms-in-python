...
class DirectedGraph:
	...
	def _dfs(self, vertex):
		...
		while dfs_stack.qsize() != 0:
			...
			if self.vertex_list[vertex].state == INITIAL:
	...
	def dfs_tree_edges(self, vertex_name):
		...
		for v in range(self.nvertices):
			p = self.vertex_list[v].predecessor
			if self.vertex_list[v].predecessor != -1:
				print("Vertex :", self.vertex_list[v].name, ", Predecessor :", self.vertex_list[p].name)
			else:
				print("Vertex :", self.vertex_list[v].name, ", Predecessor :", self.vertex_list[v].predecessor)
		
		for v in range(self.nvertices):
			u = self.vertex_list[v].predecessor
			if self.vertex_list[v].predecessor != -1:
				print("Tree Edge - (" + self.vertex_list[u].name + "," + self.vertex_list[v].name + ")")
