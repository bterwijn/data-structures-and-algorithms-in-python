...
class UndirectedGraph:
	...
	def _dfs(self, vertex, cn):
		...
		while dfs_stack.qsize() != 0:
			...
			if self.vertex_list[vertex].state == INITIAL:
				...
				self.vertex_list[vertex].component_number = cn
	...
	def connected_component(self):
		...
		component_number += 1
		self._dfs(0, component_number)	# Start DFS from vertex 0
		
		for v in range(self.nvertices):
			if self.vertex_list[v].state == INITIAL:
				component_number += 1
				self._dfs(v, component_number)
		...
		if component_number == 1:
			...
			for v in range(self.nvertices):
				print(self.vertex_list[v].name + " -> Component Number :", self.vertex_list[v].component_number)
