...
class DirectedWeightedGraph:
	...
	def _bellman_ford_algorithm(self, s):
		...
		for i in range(self.nvertices):
			self.vertex_list[i].predecessor = NIL
			self.vertex_list[i].path_length = INFINITY
			self.vertex_list[i].in_queue = False

		self.vertex_list[s].path_length = 0	# Make path_length of source vertex equal to 0
		qu.put(s)	# Insert the source vertex in the queue
		self.vertex_list[s].in_queue = True
		
		while qu.qsize() != 0:
			current = qu.get()
			self.vertex_list[current].in_queue = False

			if s == current:
				k += 1
			
			if k > self.nvertices:	# Negative cycle reachable from source vertex
				raise Exception("There is a negative cycle in graph.")

			for v in range(self.nvertices):
				if self._is_adjacent(current,v):
					if (self.vertex_list[current].path_length + self.adj[current][v]) < self.vertex_list[v].path_length:
						self.vertex_list[v].predecessor = current	# Relabel
						self.vertex_list[v].path_length = self.vertex_list[current].path_length + self.adj[current][v]
						if self.vertex_list[v].in_queue == False:
							qu.put(v)
							self.vertex_list[v].in_queue = True
	...
	def _find_path(self, s, v):
		...
		while v != s:
			path.append(v)
			u = self.vertex_list[v].predecessor
			shortest_distance += self.adj[u][v]
			v = u
	...
	def find_paths(self, source):
		...
		for v in range(self.nvertices):
			...
			if self.vertex_list[v].path_length == INFINITY:
				print("There is no path from " + source + " to vertex " + self.vertex_list[v].name)
			else:
				self._find_path(s, v)
