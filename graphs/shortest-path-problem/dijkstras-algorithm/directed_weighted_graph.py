...
class DirectedWeightedGraph:
	...
	def _get_minimum_temporary(self):
		...
		for i in range(self.nvertices):
			if self.vertex_list[i].status==TEMPORARY and self.vertex_list[i].path_length<min:
				min = self.vertex_list[i].path_length
				k = i
	...
	def _dijkstras_algorithm(self, s):
		...
		while True:
			...
			self.vertex_list[current].status = PERMANENT
			
			for v in range(self.nvertices):
				# Checks for adjacent temporary vertices
				if self._is_adjacent(current,v) and self.vertex_list[v].status==TEMPORARY:
