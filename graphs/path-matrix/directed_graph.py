...
class DirectedGraph:
	...
	def path_matrix(self):
		adjp = [ [None for column in range(self.nvertices)] for row in range(self.nvertices) ]
		x = [ [None for column in range(self.nvertices)] for row in range(self.nvertices) ]
		temp = [ [None for column in range(self.nvertices)] for row in range(self.nvertices) ]
		path = [ [None for column in range(self.nvertices)] for row in range(self.nvertices) ]
		...
		for i in range(self.nvertices):
			for j in range(self.nvertices):
				x[i][j] = adjp[i][j] = self.adj[i][j]
		
		# Get the matrix x by adding all the adjp
		for p in range(2, self.nvertices+1):
			# adjp(1...n) x adj
			for i in range(self.nvertices):
				for j in range(self.nvertices):
					temp[i][j] = 0
					for k in range(self.nvertices):
						temp[i][j] = temp[i][j] + adjp[i][k]*self.adj[k][j]
